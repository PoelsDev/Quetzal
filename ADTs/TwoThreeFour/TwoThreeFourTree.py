from ADTs.TwoThreeFour.TwoThreeFourNode import TwoThreeFourNode
    ...
    def findInorder(self, searchKey):
        ...
        *** ENKEL VOOR INTERN GEBRUIK ***
        ...
        if index == 0:  # Op basis van deze index: ga opzoek en vorm de 2 nodes om. Return de successor als deze gevonden is.
            if self.subtrees[1].getNodeAmount() is 1:
                self.subtrees[1].fix()
                if self.subtrees[1].retrieveItem(searchKey) is True:
                    return self.subtrees[1].subtrees[1].findMostLeftSuccessor()
            return self.subtrees[1].findMostLeftSuccessor()
        elif index == 1:
            if self.subtrees[2].getNodeAmount() is 1:
                self.subtrees[2].fix()
                if self.subtrees[2].retrieveItem(searchKey) is True:
                    return self.subtrees[2].subtrees[1].findMostLeftSuccessor()
            return self.subtrees[2].findMostLeftSuccessor()
        elif index == 2:
            if self.subtrees[3].getNodeAmount() is 1:
                self.subtrees[3].fix()
                if self.subtrees[3].retrieveItem(searchKey) is True:
                    return self.subtrees[3].subtrees[1].findMostLeftSuccessor()
            return self.subtrees[3].findMostLeftSuccessor()

    def findMostLeftSuccessor(self):
        if self.isLeaf():
            return self
        elif self.isLeaf() is False:
            if self.subtrees[0].getNodeAmount() is 1:
                self.subtrees[0].fix()
            self.subtrees[0].findMostLeftSuccessor()
        ...
        *** ENKEL VOOR INTERN GEBRUIK ***
        ...
        if siblingIndex[0] is not None:  # De linkersibling kan redistributen.
            ...
            if siblingIndex[1] == 0:  # De rechtersibling kan redistributen.
                ...
                if self.parent.subtrees[position-1].subtrees[self.parent.subtrees[position-1].getSubTrees()-1] is not None:
                    self.parent.subtrees[position-1].subtrees[self.parent.subtrees[position-1].getSubTrees()-1].parent = self
                self.subtrees[0] = self.parent.subtrees[position-1].subtrees[self.parent.subtrees[position-1].getSubTrees()-1]
                self.parent.subtrees[position-1].subtrees[self.parent.subtrees[position-1].getSubTrees()-1] = None
                return True
        else:
            return False
